  ...
  <div class="min-h-screen relative overflow-hidden">
    ...
    <div class="relative z-10 min-h-screen flex items-center justify-center px-4">
      <div class="text-center max-w-4xl mx-auto">
        <!-- Welcome Header -->
        <div class="floating-card p-8 mb-8 animate-float bg-black/20 backdrop-blur-sm">
          <h1 class="text-6xl md:text-8xl font-bold text-white mb-4 animate-pulse-slow">
            JCC
          </h1>
          ...
          <div class="flex flex-wrap justify-center gap-6 mb-8">
            <router-link to="/report" class="flex items-center gap-2 px-7 py-4 rounded-xl font-semibold text-lg shadow transition-all duration-300 hover:scale-105 bg-green-600/90 hover:bg-green-700/90 text-white animate-pulse-slow">
            ...
            <router-link to="/export" class="flex items-center gap-2 px-7 py-4 rounded-xl font-semibold text-lg shadow transition-all duration-300 hover:scale-105 bg-blue-600/90 hover:bg-blue-700/90 text-white animate-pulse-slow">
              <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          <!-- Total Reports Card -->
          <div class="floating-card p-6 text-center interactive-element bg-gradient-to-br from-blue-500/70 to-blue-600/70 backdrop-blur-sm animate-float animate-pulse-slow">
            <div class="text-4xl mb-4">📊</div>
            <h3 class="text-xl font-semibold text-white mb-2">Total Laporan</h3>
            <div class="text-3xl font-bold text-white mb-2">{{ stats.totalReports }}</div>
            <p class="text-blue-200 text-sm">Laporan kerusakan mesin</p>
            <div class="mt-3 text-xs text-blue-200">
              <div class="flex justify-between">
                <span>New: {{ stats.newReports }}</span>
          ...
          <!-- Total Repairs Card -->
          <div class="floating-card p-6 text-center interactive-element bg-gradient-to-br from-green-500/70 to-green-600/70 backdrop-blur-sm animate-bounce-slow animate-pulse-slow">
            <div class="text-4xl mb-4">🔧</div>
            <h3 class="text-xl font-semibold text-white mb-2">Total Perbaikan</h3>
            <div class="text-3xl font-bold text-white mb-2">{{ stats.totalRepairs }}</div>
          ...
          <div class="floating-card p-6 text-center interactive-element bg-gradient-to-br from-purple-500/70 to-purple-600/70 backdrop-blur-sm animate-pulse-slow">
            <div class="text-4xl mb-4">⚡</div>
            <h3 class="text-xl font-semibold text-white mb-2">Aktivitas Terbaru</h3>
            <div class="text-3xl font-bold text-white mb-2">{{ stats.recentActivity }}</div>
            <p class="text-purple-200 text-sm">Hari ini</p>
            <div class="mt-3 text-xs text-purple-200">
              <div class="flex justify-between">
                <span>Reports: {{ stats.todayReports }}</span>
                <span>Repairs: {{ stats.todayRepairs }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Status Display -->
        <div class="floating-card p-4 mt-8 bg-black/20 backdrop-blur-sm animate-pulse-slow">
          <div class="flex justify-between items-center">
            <div class="flex-1">
              <div class="flex items-center mb-2">
                <div class="w-3 h-3 rounded-full mr-3 animate-pulse status-indicator" :class="{
                  'bg-green-500': systemStatus === 'System Online',
                  'bg-red-500': systemStatus === 'Error loading data',
                  'bg-yellow-500': systemStatus === 'Loading data...'
                }"></div>
                <p class="text-white">
                  <span class="font-semibold gradient-text">Status Sistem:</span> 
                  <span class="ml-2 font-bold" :class="{
                    'text-green-400 animate-pulse': systemStatus === 'System Online',
                    'text-red-400 animate-pulse': systemStatus === 'Error loading data',
                    'text-yellow-400 animate-pulse': systemStatus === 'Loading data...'
                  }">{{ systemStatus }}</span>
                </p>
              </div>
              <p class="text-blue-200 text-sm">
                <span class="inline-flex items-center">
                  <span class="w-2 h-2 bg-blue-400 rounded-full mr-2 animate-ping"></span>
                  Last Updated: {{ lastUpdated }}
                </span>
                <span class="mx-2">|</span>
                <span class="inline-flex items-center">
                  <span class="w-2 h-2 bg-purple-400 rounded-full mr-2 animate-bounce"></span>
            ...
            >
              <span class="relative z-10 flex items-center">
                <span class="mr-2 group-hover:animate-spin transition-transform duration-300">🔄</span>
                Refresh
              </span>
              <div class="absolute inset-0 bg-gradient-to-r from-blue-400/50 to-purple-400/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            </button>
          </div>
        </div>
      </div>
  ...
  setup() {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const newReports = reports.filter(r => r.status === 'New' || !r.status).length
        const inProgressReports = reports.filter(r => r.status === 'Proses').length
        const todayReports = reports.filter(r => r.tanggal === today).length
        
        const totalRepairs = allRepairs.length
        const completedRepairs = allRepairs.filter(r => r.status === 'Selesai').length
        const pendingRepairs = allRepairs.filter(r => r.status === 'Pending').length
        const todayRepairs = allRepairs.filter(r => r.tanggal_selesai === today).length
    ...
    const toggleAnimation = () => {
      ...
      statusMessage.value = animationActive.value ? 'Animations enabled!' : 'Animations disabled!'
