...
export default {
  ...
  setup() {
    ...
    const fetchReports = async () => {
      const { data } = await axios.get('http://localhost:5000/api/electrical')
    ...
    const deleteRepair = async (id) => {
      if (confirm('Yakin ingin menghapus perbaikan ini?')) {
        try {
          await axios.delete(`http://localhost:5000/api/electrical/${id}`)
    ...
    const handleSubmit = async () => {
      try {
        ...
        if (form.id) {
          await axios.put(
            `http://localhost:5000/api/electrical/${form.id}`,
            cleanForm
          )
